...
Steps 9–10 built and verified the bounded `smoke21-v1` dataset: one item per class in train/val/test/calibration plus 21 FPGA bins. Step 11 completed the separately authorized one-epoch YOLOv8x-Pose-P6 gray1 integration smoke and verified a one-channel checkpoint. Step 12 reviewed the complete Git candidate set, fixed release metadata, and verified tests plus an offline wheel install. Step 13 published `main` to `https://github.com/space-exploration-101/yolo-gray1-data-pipeline` using a repository-specific write deploy key. GitHub Actions `ci` run 34482815028 passed content-guard, locked-dependency tests, and the container build. Pause before Step 14 clean-clone verification.
